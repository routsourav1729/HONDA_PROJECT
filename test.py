...
class Register:
    def __init__(self, dataset_root, split, cfg, dataset_key=None):
        ...
        self.cfg = cfg
        # Use dataset_key if provided (e.g., IDD_T2 for T2 eval), otherwise use super_split
...
if __name__ == "__main__":
    ...
    # For incremental tasks (t2, t3, t4), use the full class list (e.g., IDD_T2 for T2)
    if split_name in ['t2', 't3', 't4']:
        dataset_key = f"{task_name}_T{split_name[1].upper()}"
    else:
        dataset_key = task_name
    
    print(f"Task: {args.task}, Dataset key: {dataset_key}")
