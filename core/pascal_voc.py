...
def load_voc_instances(dirname: str, split: str, class_names: Union[List[str], Tuple[str, ...]], cfg):
    ...
    # Track unknown classes found during evaluation
    ...
    # for fileid in id2fileids.values():
        ...
        for obj in tree.findall("object"):
            ...
            # Handle class filtering for training
            if cfg.TEST.MASK and ('test' not in split):
                try:
                    if class_names.index(cls) not in allowed_class:
                        continue
                except ValueError:
                    # Class not in known classes during training - skip it
                    continue
            
            # Map to category_id, handling unknown classes for evaluation
            try:
                category_id = class_names.index(cls)
            except ValueError:
                # Class not in class_names list - map to unknown class (last index)
                if 'test' in split:
                    category_id = len(class_names) - 1  # unknown class is last in list
                    unknown_classes_found.add(cls)
                else:
                    continue  # Skip unknown classes during training
    ...
    # Log unknown classes found during evaluation
    if 'test' in split and unknown_classes_found:
        logger.info(f"\n{'='*60}")
        logger.info(f"UNKNOWN CLASSES MAPPED TO 'unknown' (index {len(class_names)-1}):")
        for cls in sorted(unknown_classes_found):
            logger.info(f"  - {cls}")
        logger.info(f"Total: {len(unknown_classes_found)} unknown classes in evaluation set")
        logger.info(f"{'='*60}\n")
