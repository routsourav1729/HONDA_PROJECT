...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def reset(self):
        ...
        self._ood_scores = defaultdict(list)
    ...
    #mmdet style bounding box
    def process_mm(self, inputs, outputs, unknown_cls=80, use_ood_score = False):
        for input, output in zip(inputs, outputs):
            #image_id = input["image_id"]
            ...
            for box, score, cls, ood_score in zip(boxes, scores, classes, ood_scores):
                if cls == -100:
                    continue
                if cls == unknown_cls:
                ...
                self._ood_scores[cls].append(
                    f"{image_id} {ood_score:.3f} {xmin:.1f} {ymin:.1f} {xmax:.1f} {ymax:.1f}"
                )
    ...
    def compute_classwise_unknown_recall(self, predictions):
        """Compute recall for each unknown class separately"""
        # Get list of images
        with PathManager.open(self._image_set_path, "r") as f:
            lines = f.readlines()
        imagenames = [x.strip() for x in lines]
        
        # Parse all annotations and collect unknown class statistics
        unknown_class_stats = defaultdict(lambda: {'total': 0, 'detected': 0})
        
        # Build mapping for image ids
        mapping = {}
        recs = {}
        ...
        # Count ground truth unknowns by class
        for imagename in recs:
            for obj in recs[imagename]:
                if obj["name"] == "unknown" and not obj["difficult"]:
                    original_class = obj["original_name"]
                    unknown_class_stats[original_class]['total'] += 1
        
        # Get predictions for unknown class
        unknown_predictions = predictions.get(self.unknown_class_index, [])
        if not unknown_predictions:
            return unknown_class_stats
        
        # Parse predictions
        splitlines = [x.strip().split(" ") for x in unknown_predictions]
        image_ids = [x[0] for x in splitlines]
        confidence = np.array([float(x[1]) for x in splitlines])
        BB = np.array([[float(z) for z in x[2:]] for x in splitlines]).reshape(-1, 4)
        
        # For each prediction, check which unknown class it matches
        for d in range(len(image_ids)):
            image_id = int(image_ids[d])
            if image_id not in mapping:
                continue
            imagename = mapping[image_id]
            bb = BB[d, :].astype(float)
            
            # Get all unknown objects in this image
            unknown_objects = [obj for obj in recs[imagename] 
                             if obj["name"] == "unknown" and not obj["difficult"]]
            
            if not unknown_objects:
                continue
            
            # Find best matching unknown object
            best_iou = 0.0
            best_obj = None
            for obj in unknown_objects:
                BBGT = np.array(obj["bbox"]).astype(float)
            ...
            # If we have a good match (IOU > 0.5), count it as detected
            if best_iou > 0.5 and best_obj is not None:
                original_class = best_obj["original_name"]
                # Mark as detected only once per GT box
                if 'matched' not in best_obj:
                    unknown_class_stats[original_class]['detected'] += 1
                    best_obj['matched'] = True
    ...
    def compute_score_distribution(self):
        known_score = []
        unknown_score = []
        annopath = self._anno_file_template
        for i in range(self.num_seen_classes):
            for line in self._ood_scores[i]:
                splitlines = line.strip().split(" ")
                image_ids = splitlines[0]
                confidence = float(splitlines[1])
                bb = [float(x) for x in splitlines[2:]]

                rec = parse_rec(annopath.format(image_ids), tuple(self.known_classes))
                for gt in rec:
                    BBGT = gt["bbox"]
                    ...
                    if gt["name"] == 'unknown':
                        unknown_score.append(confidence)
                    else:
                        known_score.append(confidence)
        return known_score, unknown_score
...
def parse_rec(filename, known_classes):
    ...
    # VOC class name mappings for COCO/VOC datasets
    # These mappings only apply to M-OWODB and S-OWODB datasets
    # IDD dataset uses its own class names without mapping
    ...
    # Determine if we're using VOC/COCO dataset based on known_classes
    # IDD has 'autorickshaw', 'rider', 'motorcycle' - VOC/COCO don't
    is_idd_dataset = 'autorickshaw' in known_classes or 'rider' in known_classes
    try:
    ...
    for obj in tree.findall("object"):
        ...
        original_cls_name = cls_name  # Keep track of original class name
        # Apply VOC class name mapping only for VOC/COCO datasets
        # IDD dataset uses its own class names directly
        if not is_idd_dataset:
            if cls_name in VOC_CLASS_NAMES_COCOFIED:
                cls_name = BASE_VOC_CLASS_NAMES[VOC_CLASS_NAMES_COCOFIED.index(cls_name)]
                original_cls_name = cls_name
        if cls_name not in known_classes:
            cls_name = 'unknown'
        obj_struct["name"] = cls_name
        obj_struct["original_name"] = original_cls_name  # Store original class name for unknown tracking
